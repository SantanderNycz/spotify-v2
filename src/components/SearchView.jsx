...
import { categories, searchAll, getArtistById, getAlbumById } from '../data/mockData';
...
function EmptySearch() {
  return (
    <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
      ...
      <p className="text-[#b3b3b3] text-sm text-center max-w-xs">Tente palavras diferentes ou verifique a grafia.</p>
    </div>
  );
}
...
export default function SearchView() {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden relative">
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(to bottom,#1a1a1a 0%,#121212 30%)' }} />
      <div className="relative flex-1 overflow-y-auto" onScroll={e => setScrolled(e.target.scrollTop > 10)}>
        ...
        <div className="px-8 pb-8">
          ...
          <div className="relative mb-8 max-w-lg">
            ...
            />
            {query && (
              <button onClick={() => { setQuery(''); setResults(null); inputRef.current?.focus(); }} className="absolute right-4 top-1/2 -translate-y-1/2 text-black/60 hover:text-black">
                <X size={18} />
              </button>
            )}
          </div>
            ...
            hasResults ? (
              <div className="animate-fade-in space-y-8">
                {results.artists.length > 0 && (
                  <section>
                    <h2 className="text-white text-2xl font-bold mb-4">Artistas</h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      {results.artists.map(a => <PlaylistCard key={a.id} item={a} isRound type="artist" />)}
                    </div>
                  </section>
                )}
                {results.tracks.length > 0 && (
                  <section>
                    <h2 className="text-white text-2xl font-bold mb-4">Músicas</h2>
                    <div>
                      {results.tracks.map((t, i) => (
                        <TrackRow key={t.id} track={t} index={i} queue={results.tracks} showAlbum />
                      ))}
                    </div>
                  </section>
                )}
                {results.playlists.length > 0 && (
                  <section>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      {results.playlists.map(p => <PlaylistCard key={p.id} item={p} />)}
                    </div>
                  </section>
                )}
                {results.albums.length > 0 && (
                  <section>
                    <h2 className="text-white text-2xl font-bold mb-4">Álbuns</h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      {results.albums.map(a => (
                        <div key={a.id} className="bg-[#181818] hover:bg-[#282828] rounded-md p-4 cursor-pointer transition-all duration-200 group">
                          <GradientCover gradient={a.gradient} className="w-full aspect-square rounded mb-4" />
                          <p className="text-white text-sm font-semibold truncate mb-1">{a.name}</p>
                          <p className="text-[#b3b3b3] text-xs">{a.year} • {getArtistById(a.artistId)?.name}</p>
                        </div>
                      ))}
                    </div>
                  </section>

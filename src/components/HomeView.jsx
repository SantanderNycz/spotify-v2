...
import { recentlyPlayed, madeForYou, playlists, artists, getPlaylistById, getArtistById } from '../data/mockData';
...
export default function HomeView() {
  ...
  const recentItems = recentlyPlayed.map(r => {
    if (r.type === 'artist') return { ...getArtistById(r.id), _type: 'artist' };
    return { ...getPlaylistById(r.id), _type: 'playlist' };
  }).filter(Boolean);

  return (
    <div className="flex flex-col flex-1 overflow-hidden relative">
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(to bottom, #1a3a2a 0%, #121212 35%)' }} />
      <div className="relative flex-1 overflow-y-auto" onScroll={e => setScrolled(e.target.scrollTop > 10)}>
        ...
        <div className="px-8 pb-8">
          ...
          <Section title="Feitos para você" onSeeAll={() => {}}>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {loading ? Array(6).fill(0).map((_,i) => <SkeletonCard key={i} />) :
                madeForYou.map(item => <PlaylistCard key={item.id} item={item} />)}
            </div>
          </Section>
          ...
          <Section title="Artistas populares" onSeeAll={() => {}}>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {loading ? Array(6).fill(0).map((_,i) => <SkeletonCard key={i} />) :
                artists.map(item => <PlaylistCard key={item.id} item={item} isRound type="artist" />)}
            </div>
          </Section>
        </div>
      </div>

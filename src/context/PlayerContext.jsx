...
import { tracks as allTracks, getArtistById, getAlbumById } from '../data/mockData';
...
export function PlayerProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const onEnded      = () => { if (skipNextRef.current) skipNextRef.current(); };
  ...
  const playQueue = useCallback((trackList, startTrack) => {
    setQueue(trackList);
  ...
  return (
    <PlayerContext.Provider value={{
      ...
      isMuted, isShuffle, repeatMode, isLiked, isLoading, queue,
      currentTimeStr: formatTime(currentTime),
      totalTimeStr:   formatTime(duration),
      play, pause, togglePlay, seek, skipNext, skipPrev,
      setVolume, setIsMuted, setIsShuffle, setRepeatMode, setIsLiked,
      playQueue,
    }}>
      {children}
